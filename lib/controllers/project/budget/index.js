...
endpoints.post('/currency', mw.createAsyncHandler(
  /**
...
endpoints.get('/:id', mw.createAsyncHandler(
  /**
   * @param {import('express').Request} req - Request object
   * @param {import('express').Response} res - Response object
   * @returns {Promise<void>}
  ...
  async (req, res) => {
    ...
    try {
      const projectBudget = await services.findProjectBudgetById(+id)
...
endpoints.post('/', mw.createAsyncHandler(
  /**
   * @param {import('express').Request} req - Request object
   * @param {import('express').Response} res - Response object
   * @returns {Promise<void>}
  ...
  async (req, res) => {
    ...
    try {
      const existingBudget = await services.findProjectBudgetById(
        req.body.projectId
      )
      if (existingBudget) {
        return res.status(400).json({
...
endpoints.put('/:id', mw.createAsyncHandler(
  /**
   * @param {import('express').Request} req - Request object
   * @param {import('express').Response} res - Response object
   * @returns {Promise<void>}
  ...
  async (req, res) => {
    ...
    const { isValid, errors } = isObjectValid(
      req.body,
      validations.updateProjectBudgetRules
    )
    ...
    try {
      const existingBudget = await services.findProjectBudgetById(+id)
      if (!existingBudget) return res.status(404).json({ success: false })

      await services.updateProjectBudgetById(+id, req.body)
...
endpoints.delete('/:id', mw.createAsyncHandler(
  /**
   * @param {import('express').Request} req - Request object
   * @param {import('express').Response} res - Response object
   * @returns {Promise<void>}
  ...
  async (req, res) => {
    ...
    try {
      ...
      await services.deleteProjectBudgetById(+id)
      res.status(200).json({ success: true })
    } catch (e) {
      res.status(500).json({ success: false, error: e.message })
    }
